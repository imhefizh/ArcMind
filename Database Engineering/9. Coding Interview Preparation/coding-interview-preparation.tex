...
\begin{itemize}
    \item Definisi: Pencarian linear adalah metode yang memeriksa setiap elemen dalam array satu per satu hingga menemukan elemen yang dicari atau mencapai akhir array.
    \item Sintaks:
          \begin{lstlisting}[language=Python, caption={}, captionpos=b]
...
\end{lstlisting}
    \item Kompleksitas Waktu:
          \begin{itemize}
              \item Best-case: O(1) (elemen ditemukan di awal)
              \item Worst-case: O(n) (elemen tidak ada atau di akhir)
          \end{itemize}
\end{itemize}

\subsection{Binary Searching}
\begin{itemize}
    \item Definisi: Pencarian biner adalah metode yang memeriksa elemen tengah dari array yang sudah terurut dan membagi ruang pencarian menjadi dua bagian berdasarkan perbandingan.
    \item Sintaks:
          \begin{lstlisting}[language=Python, caption={}, captionpos=b]
...
\end{lstlisting}
    \item Kompleksitas Waktu:
          \begin{itemize}
              \item Best-case: O(1) (elemen ditemukan di tengah)
              \item Worst-case: O(log n) (ruang pencarian dibagi dua setiap iterasi)
          \end{itemize}
\end{itemize}

\subsection{Pertimbangan dalam Searching}
\begin{itemize}
    \item Null Value: Jika nilai yang dicari tidak ada, perlu dipertimbangkan apa yang harus dikembalikan (misalnya, null).
    \item Pengembalian: Apakah pencarian harus mengembalikan instance pertama atau terakhir dari nilai yang dicari.
\end{itemize}

\subsection{Time Complexity dan Space Complexity dalam Algoritma Searching}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{./Picts/linear_search.png}
    \caption{}
\end{figure}

Ini adalah hasil yang perlu dipertimbangkan saat mengevaluasi efektivitas pencarian.
\begin{itemize}
    \item Kasus terburuk: Item tidak ada dalam daftar. Untuk menentukan ini, setiap lokasi mungkin dalam daftar berukuran n harus dicari. Kompleksitas waktu O(n).
    \item Kasus rata-rata: Elemen ditemukan di tengah. Ini dianggap sebagai hasil dengan kompleksitas waktu O(n).
    \item Kasus terbaik: Item ditemukan di indeks awal dan tidak diperlukan pemeriksaan lebih lanjut, sehingga kompleksitas waktu O(1).
    \item Kompleksitas ruang: Tidak diperlukan ruang tambahan untuk melakukan pencarian. Oleh karena itu, ruang yang diperlukan hanya sebesar jumlah item yang harus disimpan dalam daftar, kompleksitas ruang O(n).
\end{itemize}
...
\textit{Untuk melakukan pencarian biner, daftar harus terlebih dahulu diurutkan.}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{./Picts/binary_s1.png}
    \caption{}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{./Picts/binary_s2.png}
    \caption{}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{./Picts/binary_s3.png}
    \caption{}
\end{figure}

Nilai target ditemukan di sini dan tidak diperlukan pembagian lebih lanjut.

Berikut adalah hasil yang perlu dipertimbangkan saat mengevaluasi efektivitas pencarian:
\begin{itemize}
    \item Kasus terburuk: Item tidak ada dalam daftar. Karena sifat pendekatan ini, banyak item dihapus menggunakan operator logika lebih besar dan lebih kecil. Ini berarti hanya n/2 yang diperiksa pertama kali, kemudian n/4 dan n/8. Kompleksitas keseluruhan adalah O(log N).
    \item Kasus rata-rata: Elemen ditemukan setelah beberapa iterasi. Lagi-lagi karena mekanisme pencarian, setiap panggilan berikutnya mengurangi ruang keadaan. Jadi, dapat ditentukan bahwa setelah jumlah pencarian sedang, kompleksitasnya adalah O(Log N).
    \item Kasus terbaik: Item ditemukan di indeks awal dan tidak diperlukan pemeriksaan lebih lanjut, sehingga O(1).
    \item Kompleksitas ruang: Tidak diperlukan ruang tambahan untuk melakukan pencarian. Algoritma hanya menggunakan beberapa variabel (seperti target dan penghitung indeks), sehingga kompleksitas ruang adalah O(1).
    \item Catatan: Beberapa sumber mungkin menyatakan O(n) jika mereka menganggap daftar input itu sendiri sebagai ruang, tetapi secara umum kita hanya menghitung ruang tambahan yang digunakan oleh algoritma, yang adalah O(1).
\end{itemize}
...
\subsection{Prinsip Dasar Paradigma Divide and Conquer}
\begin{itemize}
    \item Divide: Input dibagi menjadi segmen-segmen yang lebih kecil dan diproses secara individu.
    \item Conquer: Setiap tugas yang terkait dengan segmen yang diberikan diselesaikan.
    \item Combine (Opsional): Menggabungkan semua segmen yang telah diselesaikan. Langkah ini tidak selalu diperlukan.
\end{itemize}

\subsubsection{Contoh: Merge Sort}
...
\subsection{Keuntungan Paradigma Divide and Conquer}
\begin{itemize}
    \item Parallelization: Memungkinkan beberapa thread atau komputer bekerja pada masalah yang sama secara bersamaan, mempercepat waktu penyelesaian.
    \item Memory Management: Mengelola memori dengan memproses data dalam potongan, terutama ketika data terlalu besar untuk dimuat dalam memori.
\end{itemize}

\subsection{Apa itu Rekursi?}
...
\subsection{Tiga Persyaratan untuk Solusi Rekursif}
\begin{itemize}
    \item Base Case: Kondisi yang memastikan fungsi tidak terus memanggil dirinya sendiri. Contoh: Jika n sama dengan 0, fungsi akan berhenti.
    \item Diminishing Structure: Memastikan bahwa input berkurang setiap kali fungsi dipanggil, sehingga akhirnya mencapai base case.
    \item Recursive Call: Memanggil fungsi itu sendiri dengan input yang lebih kecil. Contoh sintaks:
          \begin{lstlisting}[language=Python, caption={}, captionpos=b]
def exponent(x, n):
    if n == 0:
        return 1  # Base case
...
\begin{itemize}
    \item Binary Search: Fungsi yang menerima daftar dan nilai target, memeriksa titik tengah daftar, dan memanggil dirinya sendiri hingga elemen target ditemukan atau tidak ada.
    \item Fibonacci: Menghitung angka Fibonacci dengan memanggil fungsi secara rekursif untuk menjumlahkan dua angka sebelumnya.
\end{itemize}

\subsection{Keuntungan Menggunakan Rekursi}
\begin{itemize}
    \item Keterbacaan: Kode rekursif sering kali lebih mudah dibaca dibandingkan dengan loop yang kompleks.
    \item Divide and Conquer: Rekursi sering digunakan dalam pendekatan ini, membagi masalah menjadi langkah-langkah yang lebih kecil untuk menemukan solusi optimal.
\end{itemize}

\subsection{Dynamic Programming}
\begin{itemize}
    \item Definisi: Dynamic programming adalah paradigma pemrograman yang memecahkan masalah dengan membaginya menjadi sub-masalah yang lebih kecil dan menyimpan solusi untuk digunakan kembali.
    \item Keuntungan: Menghindari perhitungan ulang sub-masalah yang sama dengan mencari solusi yang sudah disimpan.
\end{itemize}

\subsection{Memoization}
\begin{itemize}
    \item Definisi: Memorization adalah teknik menyimpan hasil perhitungan dari sub-masalah untuk menghindari perhitungan ulang di masa depan.
    \item Contoh: Jika Anda menghitung \(2^3\), hasilnya disimpan dan digunakan kembali saat menghitung \(2^6\).
\end{itemize}

\subsection{Recursion}
\begin{itemize}
    \item Definisi: Recursion adalah metode pemrograman di mana fungsi memanggil dirinya sendiri untuk menyelesaikan sub-masalah.
    \item Contoh: Fungsi yang menghitung nilai Fibonacci dapat menggunakan recursion untuk memanggil dirinya sendiri.
\end{itemize}

\subsection{Contoh Masalah terkait Dynamic Programming}
\begin{itemize}
    \item Fibonacci Sequence: Contoh masalah kombinasi yang sering digunakan dalam wawancara kerja.
    \item Knapsack Problem: Masalah optimasi di mana Anda harus memilih kombinasi item dengan berat dan nilai tertentu untuk memaksimalkan nilai dalam kapasitas tertentu.
\end{itemize}

\subsection{Langkah-langkah dalam Dynamic Programming}
\begin{itemize}
    \item Tentukan Objective Function: Deskripsikan hasil optimal yang ingin dicapai.
    \item Pecah Masalah: Bagi masalah menjadi langkah-langkah yang lebih kecil.
    \item Gunakan Recursive Functions: Tulis fungsi yang dapat memanggil dirinya sendiri untuk menyelesaikan masalah.
\end{itemize}

\subsubsection{Sintaks Contoh}
...
\subsection{Prinsip Dasar Greedy Algorithm}
\begin{itemize}
    \item Occam's Razor: Prinsip filosofis yang menyatakan bahwa solusi yang paling sederhana biasanya adalah yang terbaik. Dalam konteks ini, algoritma greedy dianggap sebagai solusi yang sederhana.
    \item Greedy Algorithm: Pendekatan yang mencari solusi lokal yang terbaik pada setiap langkah, tanpa mempertimbangkan solusi global.
\end{itemize}

\subsection{Perbandingan Greedy Algorithm dengan Dynamic Programming}
\begin{itemize}
    \item Dynamic Programming: Mencari solusi optimal secara global dengan membagi masalah menjadi sub-masalah dan menyelesaikannya secara menyeluruh.
    \item Greedy Approach: Memilih opsi yang paling menguntungkan saat ini tanpa mempertimbangkan konsekuensi di masa depan.
\end{itemize}

\subsection{Contoh Penerapan Greedy Algorithm dan Dynamic Programming}
CPU Scheduling:
\begin{itemize}
    \item Dynamic Programming: Memilih subset aktivitas yang dapat diselesaikan dalam waktu tertentu.
    \item Greedy Approach: Memilih program dengan waktu eksekusi terpendek terlebih dahulu.
\end{itemize}
Knapsack Problem:
\begin{itemize}
    \item Dynamic Programming: Menentukan subset item yang memaksimalkan nilai.
    \item Greedy Approach: Memilih item dengan nilai tertinggi tanpa mempertimbangkan item lain yang mungkin lebih menguntungkan.
\end{itemize}

\subsection{Contoh Jalur Terpendek}
\begin{itemize}
    \item Dynamic Programming: Menghitung biaya dari setiap node untuk menemukan jalur terpendek secara menyeluruh.
    \item Greedy Approach: Memilih node dengan biaya terendah secara langsung tanpa menghitung semua kemungkinan.
\end{itemize}

\subsection{Kesimpulan terkait Greedy Algorithm}
Algoritma greedy menawarkan solusi yang cepat dan mudah diimplementasikan, tetapi tidak selalu menjamin hasil yang optimal. Memahami perbedaan antara kedua pendekatan ini penting untuk memilih metode yang tepat dalam menyelesaikan masalah.
